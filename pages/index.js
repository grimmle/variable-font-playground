...
export default function App({ localFonts }) {
    ...
    const loadedFonts = dbFonts
      .map((f) => {
        try {
          ...
          if (!document.fonts.has(fontFace)) document.fonts.add(fontFace);
          const fontData = buildFont(font, fontName);
          return fontData;
        } catch (error) {
          //setError(error)
          console.log(error);
  ...
  useEffect(() => {
    ...
    const ax = Object.assign(
      {},
      ...font.axes.map((axis) => {
        return { [axis.tag]: axis.defaultValue };
      })
    );
    setFontAxes(ax);
  }, [font]);
  ...
  useEffect(() => {
    ...
    const v = Object.keys(fontInstance.coordinates)
      .map((ax) => {
  ...
  const style = {
    ...
    color: color,
    fontFeatureSettings: features.map((f) => `"${f}"`).join(", "),
  };
        ...
        <div className="container">
          <div className="sidebar">
            <div
              style={{
                background:
                  "linear-gradient(-45deg, rgb(74, 65, 231) 0%, rgb(233, 33, 109) 100%)",
            ...
            >
              <Box
                sx={{
                  position: "relative",
                  background:
                    "linear-gradient(-45deg, rgb(233, 33, 109) 0%, rgb(74, 65, 231) 100%)",
                  padding: "20px 20px",
                  borderRadius: "20px",
                }}
              >
                ...
                <div className="select-container">
                  <Select
                    ...
                    renderValue={(selected) => {
                      if (!selected)
                        return <span className="placeholder">None</span>;
                  ...
                  >
                    {font.instances &&
                      font.instances.map((instance, id) => {
                        ...
                        return (
                          <MenuItem key={id} value={instance}>
                            {instance.name.en}
                          </MenuItem>
                        );
                      })}
            ...
            <div
              style={{
                background:
                  "linear-gradient(-220deg,white 0%,lightgray 20%,white 60%,gray 100%)",
                boxShadow: "0px 2px 6px rgba(0,0,0,0.3)",
                margin: "20px 0 0 0",
                padding: "10px",
                borderRadius: "25px",
            ...
            >
              <Box
                sx={{
                  padding: "10px",
                  background:
                    "linear-gradient(-40deg,white 0%,lightgray 20%,white 60%,gray 100%)",
                  padding: "15px",
                  borderRadius: "20px",
                }}
              >
                ...
                <Slider
                  ...
                  sx={{
                    "& .MuiSlider-track": {
                      background: updateGradient(10, 300, transform.size),
                    },
                  }}
                ...
                <Slider
                  size="small"
                  value={transform.letterSpace}
                  min={0}
                  max={50}
                  sx={{
                    "& .MuiSlider-track": {
                      background: updateGradient(0, 50, transform.letterSpace),
                    },
                  }}
                  valueLabelDisplay="off"
                  color="primary"
                  onChange={(e) => handleChange("letterSpace", e)}
                ...
                <Slider
                  size="small"
                  value={transform.lineHeight}
                  min={0}
                  max={3}
                  step={0.1}
                  sx={{
                    "& .MuiSlider-track": {
                      background: updateGradient(0, 3, transform.lineHeight),
                    },
                  }}
                  valueLabelDisplay="off"
                  color="primary"
                ...
                {font.axes?.map((axis) => {
                  ...
                  return (
                    <div key={axis.tag}>
                      <Label>
                        {axis.name.en} {Math.round(fontAxes[key] * 10) / 10}
                      </Label>
                      <Slider
                        size="small"
                        value={fontAxes[key] || 0}
                        ...
                        valueLabelDisplay="off"
                        onChange={(e) => handleChange(axis.tag, e)}
                        aria-label={axis.name.en}
                      ></Slider>
                    </div>
                  );
                })}
                ...
                >
                  ...
                  {font.features.map((feature) => (
                    <div
                      ...
                      style={{
                        width: "100%",
                        background: features.includes(feature.tag)
                          ? "rgb(233, 33, 109)"
                          : "",
                      }}
                      key={feature.tag}
                    >
                      <Button
                        ...
                        size="small"
                        style={{
                          color: features.includes(feature.tag)
                            ? "rgb(233, 33, 109)"
                            : "",
                        }}
                        color="primary"
          ...
          <textarea
            id="variable-text-area"
            style={style}
            defaultValue={
              loaded ? "The quick brown fox jumps over the lazy dog" : ""
            }
          ></textarea>
          <p
            style={{
